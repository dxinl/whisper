...
public:
  whisper::CommonOutput<int> run(std::nullptr_t input) override {
    std::cout << "Node1::run" << std::endl;
    return whisper::CommonOutput<int>{0, "success", 1};
  }
};

class Node2 : public whisper::Node<int, std::string> {
public:
  whisper::CommonOutput<std::string> run(int input) override {
    std::cout << "Node2::run" << std::endl;
    return whisper::CommonOutput<std::string>{0, "success",
                                              std::to_string(input)};
  }
};
...
public:
  whisper::CommonOutput<std::string> run(std::string input) override {
    std::cout << "Node3::run" << std::endl;
    return whisper::CommonOutput<std::string>{0, "success", "Node3_" + input};
  }
};

class Node4 : public whisper::Node<std::string, int> {
public:
  whisper::CommonOutput<int> run(std::string input) override {
    std::cout << "Node4::run" << std::endl;
    return whisper::CommonOutput<int>{0, "success", 4};
  }
};

class Node5 : public whisper::Node<std::tuple<std::string, int>, std::string> {
public:
  whisper::CommonOutput<std::string>
  run(std::tuple<std::string, int> input) override {
    std::cout << "Node5::run" << std::endl;
    return whisper::CommonOutput<std::string>{
        0, "success",
